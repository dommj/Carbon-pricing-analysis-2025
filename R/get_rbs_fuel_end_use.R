...
get_rbs_fuel_end_use <- function(rbs_outputs_data_file){

  #read in resisdential baseline study data
  rbs_output_cells <- xlsx_cells(rbs_outputs_data_file)
  
  #electricity consumption by end use, by state
  ...
  #clear individual data and keep total dataset
  rm(rbs_elec_end_use_by_state, rbs_natgas_end_use_by_state, rbs_lpg_end_use_by_state)
  
  return(rbs_fuel_end_use_by_state)
}
...
###############################################################
#check lpg as a proportion of total gas use and total energy
###############################################################

pct_gas_use_state <- rbs_fuel_end_use_by_state %>% 
  filter(year == 2021) %>% 
  group_by(fuel, state) %>%
  summarise(pj = sum(pj)) %>% 
  pivot_wider(names_from = fuel, values_from = pj) %>% 
  mutate(pct_lpg_gas = LPG / (LPG + `Natural Gas`),
         pct_lpg_all = LPG / (LPG + `Natural Gas` + Electricity)) 

pct_lpg_total <- rbs_fuel_end_use_by_state %>% 
  filter(year == 2021) %>% 
  group_by(fuel) %>%
  summarise(pj = sum(pj)) %>% 
  pivot_wider(names_from = fuel, values_from = pj) %>% 
...
#LPG represents less than 3 per cent of all energy use nationally and less than 6 per cent in all states. We will ignore LPG use for simplicity (revisit later)

## noting here your comment about LPG


######################################################
#convert usage between fuel types
######################################################
# 
