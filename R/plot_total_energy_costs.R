...
plot_total_energy_costs <- function(average_net_costs,
  ...
  total_energy_chart_2c <- ggplot(total_energy_chart_data_2c) +
    ...
    labs(title = "Safeguard and Reference energy costs are comparable, with RET slightly higher",
  ...
  grattan_save_all("/Users/bjjefferson/Grattan Institute Dropbox/Ben Jefferson/Apps/Overleaf/energy-2025-carbon-pricing-for-electricity/atlas/Energy Bills 2C/total_energy_costs_2C.pdf",
  ...
  total_energy_chart_data_15c <- average_net_costs %>% 
    filter(state != "WA",
           scenario == "Ref" | scenario == "1_5_Opt2" | scenario == "1_5_Opt1",
           #electrification == T is the default scenario that we use for all, showing the expected electrification
           electrification == T) %>% 
    filter(year >= 2025, year <= 2050) %>%
    group_by(year, scenario, state) %>% 
  ...
  grattan_save_all("/Users/bjjefferson/Grattan Institute Dropbox/Ben Jefferson/Apps/Overleaf/energy-2025-carbon-pricing-for-electricity/atlas/1.5C/Energy Bills",
